...
from config import surge_words, MAX_POSSIBLE_SCORE
...
class TestSurgeWords:
    def test_strong_match_scores_positive(self):
        news = pd.DataFrame([{
            "title": "Bitcoin breakout",
            "description": "Bitcoin is showing a clear breakout pattern with massive surge in volume",
        }])
        score, explanations = score_surge_words(news, surge_words)
        assert score > 0
        assert len(explanations) > 0

    def test_empty_df_scores_zero(self):
        score, explanations = score_surge_words(pd.DataFrame(), surge_words)
        assert score == 0
        assert len(explanations) == 0

    def test_irrelevant_news_scores_low(self):
        news = pd.DataFrame([{
            "title": "Weather report",
            "description": "Sunny skies expected across the region tomorrow afternoon",
        }])
        score, _ = score_surge_words(news, surge_words)
        assert score == 0
...
class TestConfig:
    def test_max_possible_score_is_21(self):
        assert MAX_POSSIBLE_SCORE == 21
